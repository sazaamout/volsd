...
void Volumes::print() {
  ...
  for (int i=0; i<counter; i++)
    std::cout << "VolId: " << vols[i].id << "\nstatu: " << vols[i].status << "\nattac: " << vols[i].attachedTo << "\nmount: " << vols[i].mountPoint << "\ndevic: " << vols[i].device << "\n";
...
bool Volumes::load(){
  ...
  while (std::getline(myFile, line)) {
      ...
      vols[counter].status = line.substr(vol_pos+1, status_pos - vol_pos);
      
      
      std::size_t mountServer_pos = line.find(' ', status_pos+1);
      vols[counter].attachedTo = line.substr(status_pos+1, mountServer_pos-status_pos);
      
      
      std::size_t mountPoint_pos = line.find(' ', mountServer_pos+1);
      vols[counter].mountPoint = line.substr(mountServer_pos+1, mountPoint_pos-mountServer_pos);
      
      
      std::size_t device_pos = line.find(' ', mountPoint_pos+1);
      vols[counter].device = line.substr(mountPoint_pos+1, device_pos-mountPoint_pos);
