...
{
  private:
	//std::map <std::string, std::string> devices_list;
  
	int transactionId;
	//utility::Volume *volumes;
  ...
  public:
	Disks (bool debug);
    Disks (bool debug, std::string rmd, std::string volumeFile);
    virtual ~Disks();
    ...
    //bool add_disk(std::string vol, std::string status, std::string ip, std::string,  std::string, int transactionId);
